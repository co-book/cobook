...
EPUBJS.Reader = function(bookPath, _options) {
	...
	if(this.settings.previousLocationCfi) {
		...
		console.log(this.settings.previousLocationCfi);
...
//cobook bookmark override by cobook
EPUBJS.Reader.prototype.addBookmark = function(cfi) {
	var chapter = this.book.toc[reader.book.currentChapter.spinePos-1].label;
	chapter = chapter.trim();
	
	console.log("Cobookmark plus :"+chapter + " // "+ cfi);
	
	var result = cobookAddBookmark(chapter,cfi);
	console.log("test1");
	console.log(result);
	var userInput = result.get('userInput');
	
	if(result.get('result')){
		//성공
		console.log("result: "+result.get('result') +" //userInput : "+ userInput);
		
		//북마크 추가 로직 
		var present = this.isBookmarked(cfi);
		if(present > -1 ) return;

		//epub view상의 bookmark 입력 (cfi 정보 , i )
		this.settings.bookmarks.push(cfi );

		
		console.log("hohoho");
		//html 추가 (html , i) 
		this.trigger("reader:bookmarked", cfi, userInput);
	}else{
		//실패
		console.log("result: "+result.get('result') +" //userInput : "+ userInput);
	}
...
EPUBJS.Reader.prototype.removeBookmark = function(cfi) {
	...
	var result = cobookRemoveBookmark(cfi);
	console.log(result);
	...
	if(result.get('result')){
		...
		console.log("result: "+result.get('result'));
		console.log("result: "+result.get('resultMsg'));
	}
...
EPUBJS.Reader.prototype.removeBookmark = function(cfi) {
	alert("EPUBJS.Reader.prototype.removeBookmark"+cfi);
	var bookmark = this.isBookmarked(cfi);
	if( bookmark === -1 ) return;
...
EPUBJS.reader.BookmarksController = function() {
	 ...
	 * override cobook
	 * */
	var createBookmarkItem = function(cfi , userInput) {
		console.log("2222");
	...
	var createBookmarkItem = function(cfi) {
		...
		//-- TODO: Parse Cfi
		//link.textContent = cfi;
		console.log("createBookmarkItem");
		var test = "<h1>text cfi /h2>"
		link.textContent = test;
		link.href = cfi;
	 ...
	 * Override cobook
	 * **/
	this.on("reader:bookmarked", function(cfi, userInput) {
		// Html (북마크 생성)  
		console.log(cfi+" "+userInput);
		var item = createBookmarkItem(cfi,userInput);
...
//	});
	...
	this.on("reader:unbookmarked", function(index) {
		...
		/*cobook add start
		var chapter = reader.book.toc[reader.book.currentChapter.spinePos-1].label;
		chapter = chapter.trim();
		var markContent = "사용자지정 문자열";
		var datastr=" \n";
		datastr+="chapter : "+chapter +" \n";
		datastr+="markContent : "+markContent +" \n";
		datastr+="cfi : "+cfi +" \n";
		
		if(cobookAddBookmark(chapter, markContent ,cfi)){
			alert("reader:bookmarked true"+datastr);
		}else{
			alert("reader:bookmarked false"+datastr);
		}
		/*cobook add end*/ 
 ...
 * */
	$bookmark.on("click", function() {
		...
		console.log("cobookmark 추가 / 제거  : "+bookmarked);
	

		if(bookmarked === -1) { //-- Add bookmark
			//cfi로 Book마크 추가 (epubjs 저장)
